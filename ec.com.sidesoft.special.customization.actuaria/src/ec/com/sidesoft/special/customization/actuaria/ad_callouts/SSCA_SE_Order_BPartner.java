package ec.com.sidesoft.special.customization.actuaria.ad_callouts;
...
public class SSCA_SE_Order_BPartner extends SimpleCallout {
  ...
  protected void execute(CalloutInfo info) throws ServletException {
    ...
    if (data != null && data.length > 0) {
      strDeliveryRule = data[0].deliveryrule.equals("")
          ...
          : data[0].deliveryrule;
      strUserRep = SSCASEOrderBPartnerData.userIdSalesRep(this, data[0].salesrepId);
      if (StringUtils.isEmpty(strUserRep)) {
        strUserRep = info.vars.getUser();
      }
      SSCASLOrderDocTypeData[] docTypeData = SSCASLOrderDocTypeData.select(this, strDocTypeTarget);
      ...
      if (strPriceList.equalsIgnoreCase("")) {
        strPriceList = SSCASEOrderBPartnerData.defaultPriceList(this, strIsSOTrx,
            info.vars.getClient());
      }
    ...
    // SSCASEOrderBPartnerData[] bpartnerData = SSCASEOrderBPartnerData.select(this, strBPartner);
    // bpartnerData[0].email_fe;

    OBCriteria<BusinessPartner> obcp = OBDal.getInstance().createCriteria(BusinessPartner.class);
    obcp.add(Restrictions.eq(BusinessPartner.ID, strBPartner));
    obcp.setMaxResults(1);

    if (obcp.list().size() > 0) {
      BusinessPartner partner = (BusinessPartner) obcp.uniqueResult();
      // double val = Double.parseDouble(costing.getCost().toString());
      if (partner.getEEIEmail() != null) {
        info.addResult("inpemEeiEmailTo", partner.getEEIEmail());
      }
    }
    ...
    if (td != null && td.length > 0) {
      ...
      String strMwarehouse = strIsSOTrx.equals("N")
          ? SSCASEOrderBPartnerData.mWarehouse(this, strBPartner)
          : SSCASEOrderBPartnerData.mWarehouseOnhand(this, strOrgId);
    ...
    info.addResult("inpisdiscountprinted",
        SSCASEOrderBPartnerData.getIsDicountPrinted(this, strBPartner));
