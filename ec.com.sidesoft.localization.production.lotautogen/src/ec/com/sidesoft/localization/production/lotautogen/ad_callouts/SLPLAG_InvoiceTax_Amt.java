 ...
 ************************************************************************
 */
package ec.com.sidesoft.localization.production.lotautogen.ad_callouts;
...
public class SLPLAG_InvoiceTax_Amt extends SimpleCallout {
  ...
  protected void execute(CalloutInfo info) throws ServletException {
    ...
    SLPLAGInvoiceTaxAmtData[] data = SLPLAGInvoiceTaxAmtData.select(this, taxId, invoiceId);
    ...
    obc.add(Restrictions.eq(TaxRate.PROPERTY_ID, taxId));
    obc.setMaxResults(1);
    TaxRate taxRateInv = (TaxRate) obc.uniqueResult();
    boolean calculate = false;
    if (taxRateInv != null) {
      if (!taxRateInv.isSlplagIrbp()) {
        calculate = true;
      }
    }
    if (calculate) {
      ...
      info.addResult("inptaxbaseamt", 0);
    }
  }
