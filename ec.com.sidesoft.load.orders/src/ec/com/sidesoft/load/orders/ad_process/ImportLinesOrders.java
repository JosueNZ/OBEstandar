...
package ec.com.sidesoft.load.orders.ad_process;
...
public class ImportLinesOrders extends IdlServiceJava {
  ...
  public BaseOBObject createOrderline(final String Linea, final String NoPedido,
    ...
    UOM ObjUOM = findDALInstance(false, UOM.class, new Value(UOM.PROPERTY_NAME, Unidad));
    ...
    if (ObjsPriceListVersion.list().size() > 0) {
      for (PriceListVersion coldelete : ObjsPriceListVersion.list()) {

        OBCriteria<ProductPrice> ObjsProductPrice = OBDal.getInstance()
            .createCriteria(ProductPrice.class);
        ObjsProductPrice.add(Restrictions.eq(ProductPrice.PROPERTY_PRICELISTVERSION, coldelete));
