  ...
  private int runAutoMatchingAlgorithm(String strReconciliationId,
    ...
    try {
      ...
      for (i = 0; i < bankLines.size(); i++) {
        ...
        List<FIN_FinaccTransaction> transactions = matched.getTransactions();
        if (transactions != null && transactions.size() > 0) {
          FIN_BankStatementLine clonedBankStatementLine = bankStatementLine;
          for (FIN_FinaccTransaction transaction : transactions) {
            if (transaction != null) {
              clonedBankStatementLine = APRM_MatchingUtility.matchBankStatementLineToTrx(
                  clonedBankStatementLine, transaction, reconciliation, matched.getMatchLevel(),
                  false);
              excluded.add(transaction);
              matchedLines++;
              // Required to persist current matching so that it is not rollbacked afterwards
              // because
              // of a future error
