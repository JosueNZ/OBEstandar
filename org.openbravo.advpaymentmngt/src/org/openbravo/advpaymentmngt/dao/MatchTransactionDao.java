...
public class MatchTransactionDao {
  ...
  public static List<FIN_FinaccTransaction> getMatchingFinancialTransaction(
    ...
    try {
      ...
      if (!"".equals(strReference) && !"**".equals(strReference)) {
        if (FIN_Utility.matchTotalAmtGroupedByRef()) {
          whereClause.append("   and (select sum(ft1.")
              .append(FIN_FinaccTransaction.PROPERTY_DEPOSITAMOUNT);
          whereClause.append(" - ft1.").append(FIN_FinaccTransaction.PROPERTY_PAYMENTAMOUNT)
              .append(")");
          whereClause.append(" from FIN_Finacc_Transaction as ft1 ");
          whereClause.append(" where ft1.").append(FIN_FinaccTransaction.PROPERTY_ACCOUNT);
          whereClause.append(".id = '").append(strFinancialAccountId).append("'");
          whereClause.append(" and (ft1.").append(FIN_FinaccTransaction.PROPERTY_FINPAYMENT);
          whereClause.append(".").append(FIN_Payment.PROPERTY_REFERENCENO);
          whereClause.append(" = :referenceNo");
          whereClause.append("   or ft1.").append(FIN_FinaccTransaction.PROPERTY_FINPAYMENT);
          whereClause.append(".").append(FIN_Payment.PROPERTY_DOCUMENTNO);
          whereClause.append(" = :documentNo))");
        } else {
          whereClause.append("   and (ft.").append(FIN_FinaccTransaction.PROPERTY_DEPOSITAMOUNT);
          whereClause.append(" - ").append(FIN_FinaccTransaction.PROPERTY_PAYMENTAMOUNT)
              .append(")");
        }
      }
      whereClause.append(" = :amount");
