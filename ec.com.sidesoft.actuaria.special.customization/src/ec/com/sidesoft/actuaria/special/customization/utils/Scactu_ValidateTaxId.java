...
public abstract class Scactu_ValidateTaxId {
	...
	static public boolean validarCedula(String numero) throws Exception {
		try {
			validarInicial(numero, 10);
			validarCodigoProvincia(numero.substring(0, 2));
			validarTercerDigito(String.valueOf(numero.charAt(2)), TIPO_CEDULA);
	...
	static public boolean validarRucPersonaNatural(String numero) throws Exception {
		try {
			validarInicial(numero, 13);
			validarCodigoProvincia(numero.substring(0, 2));
			validarTercerDigito(String.valueOf(numero.charAt(2)), TIPO_RUC_NATURAL);
	...
	static public boolean validarRucSociedadPrivada(String numero) throws Exception {
		try {
			validarInicial(numero, 13);
			validarCodigoProvincia(numero.substring(0, 2));
			...
			algoritmoModulo11(numero.substring(0, 9),
					Integer.parseInt(String.valueOf(numero.charAt(9))), RUC_PRIVADA);
		} catch (Exception e) {
			return false;
		}
		return true;
	...
	static private boolean validarInicial(String numero, int caracteres) throws Exception {
	...
	static private boolean validarCodigoProvincia(String numero) throws Exception {
	...
	static private boolean validarTercerDigito(String numero, Integer tipo) throws Exception {
		switch (tipo) {
		case 1:
		case 2:
	...
	static private boolean algoritmoModulo10(String digitosIniciales, int digitoVerificador)
			throws Exception {
	...
	static private boolean validarCodigoEstablecimiento(String numero) throws Exception {
	...
	static private boolean algoritmoModulo11(String digitosIniciales, int digitoVerificador,
			Integer tipo) throws Exception {
